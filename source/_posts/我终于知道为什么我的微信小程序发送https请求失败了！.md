...
![](./我终于知道为什么我的微信小程序发送https请求失败了！/wx-yaoqiu.png)

接下来，就是对服务器增加域名和ssl证书的环节。我先使用了阿里云的域名服务，花6块钱找了个便宜域名：

![](./我终于知道为什么我的微信小程序发送https请求失败了！/aliyun-domain.png)

接着也用它的ssl证书服务，选择了免费证书：

![](./我终于知道为什么我的微信小程序发送https请求失败了！/aliyun-ssl.png)
...
![](./我终于知道为什么我的微信小程序发送https请求失败了！/aliyun-beian.png)

### 发展阶段

接着，我的小程序就可以开始本地调试了，先运行到小程序模拟器上。结果，点击上传，开始报错了（别急，这才是噩梦的开始）：

![](./我终于知道为什么我的微信小程序发送https请求失败了！/wx-1.png)
...
![](./我终于知道为什么我的微信小程序发送https请求失败了！/wireshark-2.png)

可以看到从`TCP`的`SYN` `SYN ACK` `ACK`开始，建立https的连接需要客户端和服务端交流各种参数，确认通信无误才会结束。而我的小程序一发https请求，对应的抓包结果是：

![](./我终于知道为什么我的微信小程序发送https请求失败了！/wireshark-1.png)

只有client hello就结束了？！后面一个rst是个啥，我还不懂捏。

![](./我终于知道为什么我的微信小程序发送https请求失败了！/baidu-article1.png)
...
1. 更换ssl证书类型，大概就是选了阿里云的不同种类的证书
   ...
   ![](./我终于知道为什么我的微信小程序发送https请求失败了！/aliyun-ssl2.png)
...
3. 使用openssl检查我的nginx支持哪些https的版本
   ...
   ![](./我终于知道为什么我的微信小程序发送https请求失败了！/aliyun-server.png)

### 重点阶段

重点来了，当我在检查我的服务器支持哪些https版本时，报错信息让我又去搜索了一遍：

![](./我终于知道为什么我的微信小程序发送https请求失败了！/aliyun-sousuo.png)

这篇文章的开头就让我觉得作者是个很懂行的人：

![](./我终于知道为什么我的微信小程序发送https请求失败了！/aliyun-article1.png)

这一段确实需要有一定研究经验才能得出结论：

![](./我终于知道为什么我的微信小程序发送https请求失败了！/aliyun-article2.png)

重要信息来了：

![](./我终于知道为什么我的微信小程序发送https请求失败了！/aliyun-article3.png)
...
![](./我终于知道为什么我的微信小程序发送https请求失败了！/aliyun-server2.png)
